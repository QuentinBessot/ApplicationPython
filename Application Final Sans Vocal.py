...
class MQTTApp:
    ...
    def on_closing(self):
        ...
        if messagebox.askokcancel("Quitter", "Do you really want to quit the application  ?", icon='warning'):
            self.root.destroy()  # Ferme la fenêtre si l'utilisateur confirme
    ...
    def send_message(self):
        ...
        if message and topic:
            ...
            if messagebox.askyesno("Confirm Send", confirm_message):
                try:
                ...
                self.log_message("Message sending canceled by the user.")
        else:
    ...
    def open_new_window(self):
        ...
        def update_action_fields(event):
            ...
            elif self.action_choice.get() == "Place":
                # Cacher Move
                self.direction_label.pack_forget()
                self.direction_entry.pack_forget()
                self.distance_label.pack_forget()
    ...
    def send_action(self):
        ...
        if topic:
            ...
            if messagebox.askyesno("Confirmation", f"Do you want to send the action '{action}' to the topic '{topic}'?"):
