...
import re  # Importer pour les expressions régulières
...
# Fonction pour extraire la direction et la distance de la phrase
def extraire_direction_distance(phrase):
    # Utiliser une expression régulière pour extraire les mots "gauche" ou "droite" et un nombre (pour la distance)
    match = re.search(r"(gauche|droite|monter|descendre|avant|arrière)\s+(\d+)", phrase.lower())

    if match:
        direction = match.group(1)  # "gauche" ou "droite" ou "monter" ou "descendre" ou "avant" ou "arrière"
        distance = match.group(2)  # Nombre de centimètres
        return direction, int(distance)
    else:
        return None, None  # Si la phrase ne correspond pas au format attendu
...
def ecouter_microphone():
    ...
    with sr.Microphone() as source:
        ...
        try:
            ...
            # Vérifier si le mot "bonjour" est détecté
            if "mouvement" in texte.lower():
                afficher_popup("move")  # Affiche une popup pour "move"

                # Extraire direction et distance depuis la phrase
                direction, distance = extraire_direction_distance(texte)
                if direction is None or distance is None:
                    print("Direction ou distance non reconnue.")
                    result_label.config(text="Direction ou distance non reconnue. Veuillez réessayer.", fg="#721C24",
                                        bg="#F8D7DA")
                    return

                # Construire et envoyer le message JSON
